 ...
 * */
     ...
     * @param {number} workGroupsSize : a 3D arrays that defines the number of independents threads to be executed.
     */
    async createEncoder(description, workGroupsSize) {
        ...
        pass.dispatchWorkgroups(workGroupsSize[0], workGroupsSize[1], workGroupsSize[2]);
        pass.end();
    ...
    mainFunctionKernel(line, x, workGroupsSize) {
        ...
        if (x == 0) {
            ...
            this.workgroups_size = workGroupsSize;
            line = line.filter(item => item !== ',')
            const values = Object.values(this.inputVariablesDict);
            this.functionName = line[1]; // we stores the function name for later.
...
async function main() {
    ...
    const arraySize = 10000 * 5000;
    const workGroupSize = [256];

    const numberOfWorkGroups = [512, 256, 1];
    const generateFloat32Array = (size, value = 0.0) => new Float32Array(size).fill(value);


    const webGPUManager = new WebGPUManager();
    await webGPUManager.initialization();

    const data = generateFloat32Array(arraySize, 4);
    const data2 = generateFloat32Array(arraySize, 10);
    ...
    webGPUManager.processMain(main_kernel, workGroupSize);

    webGPUManager.graphicInterface();
    await webGPUManager.waitForConfirmation();
    webGPUManager.createBindGroup('Multiplication Matricielle');
    webGPUManager.createPipeline('Multiplication Matricielle');


    const startTime = performance.now();

    webGPUManager.createEncoder('work', numberOfWorkGroups);
    await webGPUManager.getResult();
